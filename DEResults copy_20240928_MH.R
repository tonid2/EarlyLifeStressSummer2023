...
#GSE124387 didn't have a summarized experiment object available on Gemma
#So we downloaded the Limma results for GSE124387 from Gemma
#ResultSet file "resultset_ID523692.data.txt"
#That file contains a comparison between the maternal separation group and control as well as a comparison between the maternal sepration+acupuncture group as control.
#I saved the results in a simple .txt file format with only the maternal separation vs. control comparison differential expression results included

TempResultsJoined<-read.delim("GSE124387Limma_results.txt", header=TRUE, stringsAsFactors = FALSE)
str(TempResultsJoined)
colnames(TempResultsJoined)
colnames(TempResultsJoined)[4]<-"NCBIid"
colnames(TempResultsJoined)[2]<-"GeneSymbol"
...
# [3] "Gene_Name"                     
# [4] "NCBIid"                        
# [5] "FoldChange_maternal.separation"
# [6] "Tstat_maternal.separation"     
# [7] "PValue_maternal.separation" 
...
TempResultsJoined_NoNA_NoMultimapped_FoldChanges<-cbind(TempResultsJoined_NoNA_NoMultimapped$FoldChange_maternal.separation)

str(TempResultsJoined_NoNA_NoMultimapped_FoldChanges)
#num [1:17233, 1] -3.649 0.877 0.495 0.654 3.528 ...
...
TempResultsJoined_NoNA_NoMultimapped_Tstats<-cbind(TempResultsJoined_NoNA_NoMultimapped$Tstat_maternal.separation)

str(TempResultsJoined_NoNA_NoMultimapped_Tstats)
#num [1:17233, 1] -11.31 10.04 8.15 7 6.96 ...

row.names(TempResultsJoined_NoNA_NoMultimapped_Tstats)<-TempResultsJoined_NoNA_NoMultimapped$NCBIid

#Let's rename our columns to something nicer describing the effect of interest:

ComparisonsOfInterest<-c("GSE124387SeparationVS_Ctrl")
...
ExtractingDEResults(GSE_ID="GSE124387", TempResultsJoined_NoNA_NoMultimapped_FoldChanges, TempResultsJoined_NoNA_NoMultimapped_Tstats)

##

#This part is not dataset specific:

str(DEResults_GSE124387)
